...
## Демо сборки корзины X5

В каталоге `nutrition_bot/` лежит прототип, который по меню автоматически подбирает товары и собирает корзину для магазинов X5 (например, «Пятёрочка»). В репозитории уже есть все необходимые файлы и данные, чтобы быстро попробовать демо без доступа к реальному API.

### Что уже входит в репозиторий

- `nutrition_bot/data/sample_menu.json` — пример меню из трёх ингредиентов.
- `nutrition_bot/data/x5_sample_catalog.json` — фрагмент каталога X5 с ценами и SKU.
- `nutrition_bot/x5_cart.py` — логика подбора товаров и расчёта упаковок.
- `nutrition_bot/x5_api.py` — асинхронный клиент для неофициального API X5 (аутентификация, загрузка каталога, отправка корзины).
- `nutrition_bot/x5_cli.py` — CLI-интерфейс для ручных экспериментов.
- `scripts/run_x5_demo.sh` — удобный скрипт, запускающий CLI с готовыми демо-данными (в корне репозитория есть обёртка `run_x5_demo.sh`).

### Как запустить демо локально

1. (Опционально) создайте виртуальное окружение Python 3.10+.
...
3. Выполните скрипт демо (выведет краткий отчёт и JSON-пейлоад корзины):

   ```bash
   ./run_x5_demo.sh
   ```

   При необходимости передайте дополнительные аргументы CLI (например, `--catalog` или `--submit`) прямо в команду, они будут проксированы скриптом.

### Использование реального API X5

Если у вас есть доступ к внутреннему (неофициальному) API X5:

```bash
python -m nutrition_bot.x5_cli \
  --menu nutrition_bot/data/sample_menu.json \
  --use-api-catalog \
  --api-base https://example.x5.ru \
  --store-id 12345 \
  --phone +79998887766 \
  --password 0000 \
  --submit
...
CLI загрузит каталог, подберёт товары, сформирует корзину и отправит её на сервер. Оплата всегда остаётся за пользователем.
